...
public class LeaveRequests_RejectedAdapter extends RecyclerView.Adapter<LeaveRequests_RejectedAdapter.ViewHolder> {
    ...
    public LeaveRequests_RejectedAdapter(Context context, JSONArray data) {
    ...
    @NonNull
    @Override
    public LeaveRequests_RejectedAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(mContext).
                inflate(R.layout.leaverequest_pending_details_item, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull LeaveRequests_RejectedAdapter.ViewHolder holder, int position) {
        ...
        if (mData != null) {
            if (mData.length() > 0) {
                try {
                    ...
                    holder.leaveStatus.setTextColor(mContext.getResources().getColor(R.color.red));
                    holder.editLayout.setOnClickListener(clickedView -> {
                        newPosition = holder.getAdapterPosition();
                        System.out.println("view holder newPosition: " + newPosition);
                        try {
                            if (datajson.get("requestid") != null) {
                                approveAPI(datajson.get("requestid").toString());
                            }
    ...
    private void approveAPI(String requestid) {
        String
